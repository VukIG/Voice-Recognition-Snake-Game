...
function App() {
  ...
  function isGameOver() {
    return snake[0].x >= size || snake[0].x < 0 || snake[0].y >= size || snake[0].y < 0;
  }
      ...
      for (let j = 0; j < size; ++j) {
        ...
        for(let block of snake) {
          if (block.x === i && block.y === j) {
            added = true;
            field.push(false, true);
          } 
        }
        if (!added) {
          if (apple.x === i && apple.y === j) {
            field.push(true, false);
          } else {
            field.push(false, false);
          }
        }
        fields.push(field);
      }
    ...
    <div className="h-screen w-screen flex flex-col gap-10 items-center justify-center bg-gray-200">
      <div className="bg-white grid grid-cols-10 grid-rows-10 gap-0 aspect-square h-1/2">
        {renderGame().map((field,index) => (
          <Field key={index} className="col-span-1 row-span-1 bg-black" apple={field[0]} snakeHead={field[1]} />
        ))}
      </div>
