...
function App() {
  ...
  function updateDirection(event) {
    if (event == 'up' && direction.x != 1) setDirection({ x: -1, y: 0 });
    else if (event == 'down' && direction.x != -1) setDirection({ x: 1, y: 0 });
    else if (event == 'left' && direction.y != 1) setDirection({ x: 0, y: -1 });
  ...
  return (
    <div className="h-screen w-screen flex flex-col gap-10 items-center justify-center bg-gray-200">
      ...
      <div className=" flex flex-col gap-3">
        <button
          className=" text-xl p-3 rounded bg-green-700 hover:bg-green-900 transition text-white"
          onClick={recordSpeech}
        >
          Press to speak
        </button>
        <p className="text-small">
          Note * You can stop the recording by using the voice command: 'stop'
        </p>
